...
const FloatingPills = dynamic(() => import('@/components/three/FloatingPills'), { ssr: false });
...
export default function DashboardPage() {
  ...
  if (loading) {
    return (
      <div className="p-6 space-y-6">
        ...
        <div className="h-32 rounded-2xl bg-surface-200 dark:bg-surface-800 animate-pulse" />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2 h-64 rounded-2xl bg-surface-200 dark:bg-surface-800 animate-pulse" />
          <div className="h-64 rounded-2xl bg-surface-200 dark:bg-surface-800 animate-pulse" />
        </div>
      </div>
    );
  }
  ...
  return (
    <motion.div
      ...
      animate="visible"
    >
      {/* ─── Welcome Banner with Particles + FloatingPills ─── */}
      <motion.div variants={staggerItem}>
        <BlurFade delay={0.05}>
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-surface-900 via-surface-950 to-surface-900 dark:from-surface-800 dark:via-surface-900 dark:to-surface-800 p-8 text-white">
            <Particles
        ...
        {kpiConfig.map((kpi, index) => {
          ...
          return (
            <BlurFade key={kpi.key} delay={0.1 + index * 0.08}>
              <motion.div variants={staggerItem}>
                <MagicCard
                  className="cursor-default border-surface-200 bg-white dark:border-surface-700 dark:bg-surface-900"
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="mt-2 flex items-baseline gap-1">
                          {isRevenue && (
                            <span className="text-2xl font-extrabold text-surface-900 dark:text-surface-100 tracking-tight">{'\u20A6'}</span>
                          )}
                          <NumberTicker
                            value={typeof value === 'number' ? value : 0}
                            delay={0.2 + index * 0.1}
                            className="text-2xl font-extrabold text-surface-900 dark:text-surface-100 tracking-tight"
                        ...
                        {isRevenue && stats.revenueChange && (
                          <p className={cn(
                            ...
                            stats.revenueChange.startsWith('+') ? 'text-brand-600' : stats.revenueChange.startsWith('-') ? 'text-danger-500' : 'text-surface-400 dark:text-surface-500'
                          )}>
                            {stats.revenueChange}
                          </p>
                        )}
      ...
      <motion.div variants={staggerItem} className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <BlurFade delay={0.35} className="xl:col-span-2">
          <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-200 dark:border-surface-700 shadow-card overflow-hidden">
            <div className="px-5 py-4 border-b border-surface-200 dark:border-surface-700 flex items-center justify-between">
              <h3 className="font-bold text-surface-900 dark:text-surface-100">Recent Orders</h3>
              <a href="/orders" className="text-sm font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300 flex items-center gap-1 transition-colors">
                View all <ArrowUpRight className="w-3.5 h-3.5" />
              </a>
            </div>
            {recentOrders.length === 0 ? (
              <div className="px-5 py-12 text-center">
                <ShoppingCart className="w-10 h-10 text-surface-300 dark:text-surface-600 mx-auto mb-3" />
                <p className="text-sm text-surface-400 dark:text-surface-500">No orders yet. Place your first order from the Supplier Catalog.</p>
              </div>
            ) : (
              <AnimatedList delay={800} className="!gap-0">
                {recentOrders.map((order: any) => (
                  <div
                    key={order.id}
                    className={cn(
                      'px-5 py-3.5 flex items-center justify-between transition-all duration-200 hover:bg-surface-50 dark:hover:bg-surface-800 border-l-[3px] border-b border-b-surface-100 dark:border-b-surface-800',
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-surface-100 dark:bg-surface-800 rounded-lg flex items-center justify-center">
                        <ShoppingCart className="w-4 h-4 text-surface-500 dark:text-surface-400" />
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-sm font-bold text-surface-800 dark:text-surface-200">{formatCurrency(order.total_amount)}</span>
        ...
        <BlurFade delay={0.4}>
          <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-200 dark:border-surface-700 shadow-card overflow-hidden">
            <div className="px-5 py-4 border-b border-surface-200 dark:border-surface-700 flex items-center justify-between">
              <h3 className="font-bold text-surface-900 dark:text-surface-100">Low Stock Alert</h3>
              <a href="/inventory" className="text-sm font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300 flex items-center gap-1 transition-colors">
                View all <ArrowUpRight className="w-3.5 h-3.5" />
              </a>
            </div>
            {lowStock.length === 0 ? (
              <div className="px-5 py-12 text-center">
                <Package className="w-10 h-10 text-surface-300 dark:text-surface-600 mx-auto mb-3" />
                <p className="text-sm text-surface-400 dark:text-surface-500">No low stock items.</p>
              </div>
            ) : (
              <div className="divide-y divide-surface-100 dark:divide-surface-800">
                {lowStock.map((item: any, i: number) => {
                  ...
                  return (
                    <div key={i} className="px-5 py-3.5 hover:bg-surface-50 dark:hover:bg-surface-800 transition-colors">
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm font-semibold text-surface-800 dark:text-surface-200">{item.product_name}</p>
                        </div>
                        <span className={cn(
                      ...
                      <div className="w-full h-2 bg-surface-100 dark:bg-surface-800 rounded-full overflow-hidden">
                        <motion.div
                          ...
                          className={cn('h-full rounded-full', isCritical ? 'bg-gradient-to-r from-red-500 to-red-400' : 'bg-gradient-to-r from-amber-500 to-amber-400')}
                        />
                      </div>
                    </div>
                  );
            ...
            <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-200 dark:border-surface-700 shadow-card overflow-hidden">
              <div className="px-5 py-4 border-b border-surface-200 dark:border-surface-700 flex items-center justify-between">
                <h3 className="font-bold text-surface-900 dark:text-surface-100 flex items-center gap-2">
                  <MessageSquare className="w-4 h-4 text-brand-600 dark:text-brand-400" /> Active Consultations
                </h3>
                <a href="/consultations" className="text-sm font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300 flex items-center gap-1 transition-colors">
                  View all <ArrowUpRight className="w-3.5 h-3.5" />
                </a>
              </div>
              <div className="divide-y divide-surface-100 dark:divide-surface-800">
                {consultations.map((c: any) => {
                  ...
                  return (
                    <div
                      key={c.id}
                      className="px-5 py-3.5 flex items-center justify-between transition-all duration-200 hover:bg-surface-50 dark:hover:bg-surface-800"
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-gradient-to-br from-brand-100 to-brand-200 dark:from-brand-800 dark:to-brand-700 rounded-full flex items-center justify-center">
                          <span className="text-xs font-bold text-brand-700 dark:text-brand-200">{initials}</span>
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-surface-800 dark:text-surface-200">Consultation</p>
                          <p className="text-xs text-surface-400 dark:text-surface-500">{c.symptom_summary || 'In progress...'}</p>
                        </div>
                      </div>
