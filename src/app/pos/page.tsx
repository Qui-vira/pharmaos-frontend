        ...
        <div className="p-6 flex items-center justify-center min-h-[calc(100vh-4rem)]">
          ...
          >
            ...
            <motion.div
              ...
              className="w-20 h-20 bg-gradient-to-br from-brand-100 to-brand-200 rounded-full flex items-center justify-center mx-auto mb-5 shadow-lg shadow-brand-200/40"
            >
              <motion.div
            ...
            <div className="bg-surface-50 rounded-xl divide-y divide-surface-200 mb-5 text-left overflow-hidden">
              {receipt.items.map((item, i) => (
                ...
                >
                  ...
                  <p className="text-sm font-bold text-surface-700">{formatCurrency(item.quantity * item.unit_price)}</p>
                </motion.div>
              ))}
            </div>
            ...
            <motion.div
              ...
              className="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-brand-50 to-brand-100/60 rounded-xl mb-5 border border-brand-200/40"
            >
              <span className="font-bold text-surface-900">Total</span>
              <span className="text-2xl font-extrabold text-brand-700">{formatCurrency(receipt.total)}</span>
            </motion.div>
            ...
            <motion.p
              ...
              className="text-sm text-surface-500 mb-6 capitalize"
            >
              Paid via <strong className="text-surface-700">{receipt.payment}</strong>
            </motion.p>
    ...
    <>
      ...
      <div className="p-6">
        <div className="flex gap-6 h-[calc(100vh-8rem)]">
            ...
            >
              ...
              >
                <Search className={cn(
                  ...
                  searchFocused ? 'text-brand-500' : 'text-surface-400',
                )} />
              </motion.div>
              <input
                ref={searchRef}
              ...
              <AnimatePresence>
                {search && (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.85 }}
                    onClick={() => setSearch('')}
                    className="w-7 h-7 rounded-full bg-surface-100 hover:bg-surface-200 flex items-center justify-center text-surface-500 transition-colors"
                  >
                      ...
                      <motion.button
                        ...
                        className={cn(
                          ...
                          outOfStock
                            ...
                            : 'hover:shadow-elevated hover:bg-gradient-to-br hover:from-white hover:to-brand-50/40 border-surface-200 hover:border-brand-300',
                          inCart && 'border-brand-400 bg-gradient-to-br from-white to-brand-50/30 shadow-md shadow-brand-100/30',
                        )}
                      >
                        {/* Category chip */}
                        {item.product?.category && (
                          <span className="inline-block text-[10px] font-semibold uppercase tracking-wider text-brand-600 bg-brand-50 rounded-full px-2 py-0.5 mb-2.5">
                            {item.product.category}
                          </span>
                        )}
                        ...
                        <div className="flex items-center justify-between mt-3.5">
                          ...
                          <span className="flex items-center gap-1.5 text-xs font-semibold">
                            ...
                            <span className={cn(
                              outOfStock ? 'text-danger-500' : lowStock ? 'text-warning-600' : 'text-surface-400',
                            )}>
          ...
          {/* ─── Right: Cart ─── */}
          <div className="w-[400px] flex-shrink-0 rounded-2xl border border-surface-200 bg-white shadow-lg flex flex-col overflow-hidden">
            {/* Cart header — sticky with blur */}
            <div className="sticky top-0 z-10 px-5 py-4 border-b border-surface-200 bg-white/80 backdrop-blur-lg flex items-center justify-between">
              <h3 className="font-bold text-surface-900 flex items-center gap-2.5">
              ...
              <AnimatePresence>
                {cart.length > 0 && (
                  <motion.span
                    ...
                    animate={{ scale: 1 }}
                    exit={{ scale: 0 }}
                    className="bg-brand-100 text-brand-700 text-xs font-bold px-2.5 py-1 rounded-full"
                  >
            ...
            <div className="flex-1 overflow-y-auto">
              {cart.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-surface-400">
                  ...
                  <p className="text-xs text-surface-300 mt-1">Items will appear here</p>
                </div>
              ) : (
                    ...
                    >
                      <div className="px-5 py-3.5">
                        <div className="flex items-start justify-between gap-2 mb-2.5">
                          <p className="text-sm font-bold text-surface-800 leading-tight">{item.name}</p>
                          <motion.button
                            whileHover={{ scale: 1.2 }}
                            whileTap={{ scale: 0.85 }}
                            onClick={() => removeFromCart(item.product_id)}
                            className="w-6 h-6 rounded-full flex items-center justify-center text-surface-400 hover:text-danger-500 hover:bg-danger-50 transition-colors flex-shrink-0"
                          >
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1.5">
                            <motion.button
                              ...
                              className="w-8 h-8 rounded-full bg-surface-100 hover:bg-surface-200 flex items-center justify-center disabled:opacity-30 transition-colors"
                            >
            ...
            <AnimatePresence>
              {cart.length > 0 && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 20 }}
                  className="border-t border-surface-200 bg-surface-50/50 p-5 space-y-4"
                >
                  {error && (
                    <motion.div
                      initial={{ opacity: 0, y: -5 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="p-3 bg-danger-500/10 text-danger-600 text-xs font-medium rounded-xl border border-danger-500/20"
                    >
                      {error}
                    </motion.div>
                  )}
                  ...
                  {/* Total */}
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-semibold text-surface-500">Total</span>
                    <motion.span
                      key={cartTotal}
                      initial={{ scale: 1.08, y: -2 }}
                      animate={{ scale: 1, y: 0 }}
                      transition={{ type: 'spring', stiffness: 300, damping: 20 }}
                      className="text-2xl font-extrabold text-surface-900"
                    >
                    ...
                    {[
                      ...
                      <motion.button
                        ...
                        className={cn(
                          ...
                          paymentMethod === key
                            ? 'border-brand-500 bg-brand-50 text-brand-700 shadow-sm shadow-brand-200/30'
